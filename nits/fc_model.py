...
class ResMADEModel(nn.Module):
    ...
    def forward(self, x, mask_train_phase=False, num_reduce_dims=[]):
        ...
        if mask_train_phase:
            ...
            ll, recon = self.forward_vec(x, num_reduce_dims)
            with torch.no_grad():
                ll_aug, _ = self.forward_vec(x_aug, num_reduce_dims)
            return ll, mask, ll_aug, recon
        else:
    ...
    def forward_vec(self, x, num_reduce_dims=[]):
        ...
        # compute log likelihood
        grad = self.nits_model.pdf(x, params)
        ll = (grad + 1e-10).log()
        if len(num_reduce_dims):
